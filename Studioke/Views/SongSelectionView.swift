...
struct SongSelectionView: View {
    ...
    var filteredSongs: [Song] {
        return sampleSongs.filter { song in
            let filterMatches = selectedFilter.map { song.tags.contains($0.title) } ?? true
            let searchMatches = searchText.isEmpty || song.title.localizedStandardContains(searchText)
